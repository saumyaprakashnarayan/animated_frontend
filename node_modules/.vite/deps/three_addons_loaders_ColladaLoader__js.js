import { AmbientLight, AnimationClip, Bone, BufferGeometry, ClampToEdgeWrapping, Color, DataTextureLoader, DirectionalLight, FileLoader, Float32BufferAttribute, Group, Line, LineBasicMaterial, LineSegments, LinearMipmapLinearFilter, Loader, LoaderUtils, MathUtils, Matrix4, Mesh, MeshBasicMaterial, MeshLambertMaterial, MeshPhongMaterial, OrthographicCamera, PerspectiveCamera, PointLight, Quaternion, QuaternionKeyframeTrack, RepeatWrapping, SRGBColorSpace, Scene, Skeleton, SkinnedMesh, SpotLight, TextureLoader, Vector2, Vector3, VectorKeyframeTrack } from "./three.js";
	...
	parse(buffer) {
		function tgaCheckHeader(header) {
			switch (header.image_type) {
				...
				default: throw new Error("THREE.TGALoader: Invalid type " + header.image_type);
			}
		...
		switch (header.image_type) {
			...
			case TGA_TYPE_RLE_GREY:
				...
				use_grey = true;
				break;
			case TGA_TYPE_GREY: use_grey = true;
		}
...
var ColladaLoader = class extends Loader {
	...
	parse(text, path) {
		...
		function parseAssetUnit(xml) {
			if (xml !== void 0 && xml.hasAttribute("meter") === true) return parseFloat(xml.getAttribute("meter"));
			else return 1;
		}
		...
		function parseAnimation(xml) {
			...
			if (hasChildren === false) library.animations[xml.getAttribute("id") || MathUtils.generateUUID()] = data;
		}
		...
		function parseAnimationChannel(xml) {
			const data = {};
			let parts = xml.getAttribute("target").split("/");
		...
		function buildAnimationChannel(channel, inputSource, outputSource) {
			...
			switch (transform) {
				...
				case "rotate":
					console.warn("THREE.ColladaLoader: Animation transform type \"%s\" not yet implemented.", transform);
					break;
				case "scale": console.warn("THREE.ColladaLoader: Animation transform type \"%s\" not yet implemented.", transform);
			}
		...
		function parseAnimationClip(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "instance_animation": data.animations.push(parseId(child.getAttribute("url")));
				}
			}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "joints":
						...
						break;
					case "vertex_weights": data.vertexWeights = parseVertexWeights(child);
				}
			}
			return data;
		}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "vcount":
						...
						break;
					case "v": data.v = parseInts(child.textContent);
				}
			}
			return data;
		}
		...
		function parseImage(xml) {
			const data = { init_from: getElementsByTagName(xml, "init_from")[0].textContent };
		...
		function parseEffect(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "profile_COMMON": data.profile = parseEffectProfileCOMMON(child);
				}
			}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "technique":
						...
						break;
					case "extra": data.extra = parseEffectExtra(child);
				}
			}
			return data;
		}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					case "surface":
						...
						break;
					case "sampler2D": data.samplers[sid] = parseEffectSampler(child);
				}
			}
		}
		function parseEffectSurface(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "init_from": data.init_from = child.textContent;
				}
			}
			return data;
		}
		function parseEffectSampler(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "source": data.source = child.textContent;
				}
			}
			return data;
		}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "transparency":
						...
						break;
					case "transparent": data[child.nodeName] = {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "float":
						...
						break;
					case "texture": data[child.nodeName] = {
		...
		function parseEffectParameterTexture(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "extra": parseEffectParameterTextureExtra(child, data);
				}
			}
			return data;
		}
		function parseEffectParameterTextureExtra(xml, data) {
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "technique": parseEffectParameterTextureExtraTechnique(child, data);
				}
			}
		}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "wrapV":
						...
						break;
					case "bump": data[child.nodeName] = parseEffectExtraTechniqueBump(child);
				}
			}
		}
		function parseEffectExtra(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "technique": data.technique = parseEffectExtraTechnique(child);
				}
			}
			return data;
		}
		...
		function parseMaterial(xml) {
			const data = { name: xml.getAttribute("name") };
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				const child = xml.childNodes[i];
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "instance_effect": data.url = parseId(child.getAttribute("url"));
				}
			}
		...
		function buildMaterial(data) {
			...
			switch (technique.type) {
				...
				case "lambert":
					...
					break;
				default: material = new MeshBasicMaterial();
			}
			...
			if (transparent && transparency) {
				if (transparent.data.texture) material.transparent = true;
				else {
		...
		function parseCamera(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "optics": data.optics = parseCameraOptics(child);
				}
			}
		...
		function parseCameraParameters(xml) {
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					...
					case "znear":
					case "zfar":
					case "aspect_ratio": data[child.nodeName] = parseFloat(child.textContent);
				}
			}
			return data;
		}
		function buildCamera(data) {
			...
			switch (data.optics.technique) {
				...
				case "orthographic":
					...
					break;
				default: camera = new PerspectiveCamera();
			}
		...
		function parseLight(xml) {
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "technique_common": data = parseLightTechnique(child);
				}
			}
		...
		function buildLight(data) {
			...
			switch (data.technique) {
				...
				case "spot":
					light = new SpotLight();
					break;
				case "ambient": light = new AmbientLight();
			}
			...
			for (let i = 0, l = xml.childNodes.length; i < l; i++) {
				...
				switch (child.nodeName) {
					...
					case "vcount":
						...
						break;
					case "p": primitive.p = parseInts(child.textContent);
				}
			}
			return primitive;
		}
		...
		function checkUVCoordinates(primitives) {
			let count = 0;
			for (let i = 0, l = primitives.length; i < l; i++) if (primitives[i].hasUV === true) count++;
						...
						for (let g = 0; g < primitive.count; g++) {
							...
							switch (vc) {
								...
								case 4:
									count += 6;
									break;
								default: count += (vc - 2) * 3;
							}
						}
						break;
					default: console.warn("THREE.ColladaLoader: Unknow primitive type:", primitive.type);
				}
								...
								switch (key) {
									case "POSITION":
										...
										if (primitive.hasUV === false && primitives.uvsNeedsFix === true) {
											...
											for (let i = 0; i < count; i++) uv.array.push(0, 0);
										}
										break;
									case "NORMAL":
		...
		function parseKinematicsModel(xml) {
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "technique_common": parseKinematicsTechniqueCommon(child, data);
				}
			}
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					case "joint":
						...
						break;
					case "link": data.links.push(parseKinematicsLink(child));
				}
			}
		}
		function parseKinematicsJoint(xml) {
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					case "prismatic":
					case "revolute": data = parseKinematicsJointParameter(child);
				}
			}
			return data;
		}
		...
		function parseKinematicsLink(xml) {
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					...
					case "matrix":
					case "translate":
					case "rotate": data.transforms.push(parseKinematicsTransform(child));
				}
			}
			return data;
		}
		...
		function parsePhysicsRigidBody(xml, data) {
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "technique_common": parsePhysicsTechniqueCommon(child, data);
				}
			}
		}
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					case "inertia":
						...
						break;
					case "mass": data.mass = parseFloats(child.textContent)[0];
				}
			}
		}
		function parseKinematicsScene(xml) {
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
					case "bind_joint_axis": data.bindJointAxis.push(parseKinematicsBindJointAxis(child));
				}
			}
			library.kinematicsScenes[parseId(xml.getAttribute("url"))] = data;
		}
		function parseKinematicsBindJointAxis(xml) {
			const data = { target: xml.getAttribute("target").split("/").pop() };
			for (let i = 0; i < xml.childNodes.length; i++) {
				const child = xml.childNodes[i];
				if (child.nodeType !== 1) continue;
				switch (child.nodeName) {
							...
							for (let i = 0; i < transforms.length; i++) {
								...
								if (transform.sid && transform.sid.indexOf(jointIndex) !== -1) switch (joint.type) {
									...
									case "prismatic":
										...
										break;
									default: console.warn("THREE.ColladaLoader: Unknown joint type: " + joint.type);
								}
									...
									case "scale":
										...
										break;
									case "rotate": matrix.multiply(m0.makeRotationAxis(transform.obj, transform.angle));
								}
							}
			...
			for (let i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					case "bind_material":
						...
						break;
					case "skeleton": data.skeletons.push(parseId(child.textContent));
				}
			}
			return data;
		}
		...
		function buildObjects(geometries, instanceMaterials) {
			...
			for (const type in geometries) {
				...
				if (materials.length === 0) {
					if (type === "lines" || type === "linestrips") materials.push(new LineBasicMaterial());
				...
				switch (type) {
					...
					case "linestrips":
						...
						break;
					case "triangles":
					case "polylist": if (skinning) object = new SkinnedMesh(geometry.data, material);
